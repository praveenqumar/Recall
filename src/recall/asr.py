...
def _faster(wav: Path, language: Optional[str], chunk_s: float, model: str,
    ...
    seg_iter, _info = m.transcribe(
        ...
        # threshold 0.2 is proven on real (soft/accented) Hinglish — 0.45 dropped
        # ALL speech on a real file. Keep only the safe anti-loop add: cap segment
        # length so a runaway decode can't grow into a 500-word loop.
        vad_parameters={"threshold": 0.2, "min_speech_duration_ms": 100,
                        "min_silence_duration_ms": 500, "max_speech_duration_s": 15,
                        "speech_pad_ms": 400},
